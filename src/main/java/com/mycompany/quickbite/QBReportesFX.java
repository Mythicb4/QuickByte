...
public class QBReportesFX implements Initializable {
    ...
    // Tablas
    @FXML
    private TableView<ProductSalesStats> tblMasVendidos;
    @FXML
    private TableColumn<ProductSalesStats, String> colName;
    @FXML
    private TableColumn<ProductSalesStats, Double> colCosto; // Asumo que el nombre en FXML es solo "Costo"
    @FXML
    private TableColumn<ProductSalesStats, Integer> colStock; // No tiene sentido en esta tabla, pero lo mapeamos
    @FXML
    private TableColumn<ProductSalesStats, String> colTotalVentas; // El ID colProvider se usó para "Ventas Totales"

    @FXML
    private TableView<ProductSalesStats> tblMenosVendidos;
    @FXML
    private TableColumn<ProductSalesStats, String> colName1;
    @FXML
    private TableColumn<ProductSalesStats, Double> colCosto1; // Asumo que el nombre en FXML es solo "Costo"
    @FXML
    private TableColumn<ProductSalesStats, Integer> colStock1;
    @FXML
    private TableColumn<ProductSalesStats, String> colTotalVentas1; // El ID colProvider1 se usó para "Ventas Totales"
    ...
    private void configureTable(TableView<ProductSalesStats> tableView,
        ...
        costCol.setCellValueFactory(new PropertyValueFactory<>("totalCost")); // Mostramos el costo total, no el
                                                                              // unitario
    ...
    private void loadReportData(LocalDate start, LocalDate end) {
        ...
        List<ProductSalesStats> mostSold = summary.productSalesStats().stream()
                .sorted(Comparator.comparingInt((ProductSalesStats stats) -> stats.getTotalQuantitySold()).reversed()) // ⬅️
                                                                                                                       // CAMBIO
                                                                                                                       // AQUÍ
                .limit(10)
        ...
        List<ProductSalesStats> leastSold = summary.productSalesStats().stream()
                .sorted(Comparator.comparingInt((ProductSalesStats stats) -> stats.getTotalQuantitySold())) // ⬅️ CAMBIO
                                                                                                            // AQUÍ
                .limit(10)
                .collect(Collectors.toList());
        tblMenosVendidos.setItems(FXCollections.observableList(leastSold));

        // Nota: Las columnas Stock y Costo en ambas tablas ahora reflejan los valores
    ...
    public record ProductSalesStats(
        ...
        // Métodos auxiliares para FXCollections (aunque no estrictamente necesarios
        // para Records)
        ...
        // Método calculado para obtener el costo total (costo unitario × cantidad
        // vendida)
        public double getTotalCost() {
            return cost * totalQuantitySold;
        }
    }
