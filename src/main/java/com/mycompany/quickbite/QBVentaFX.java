...
public class QBVentaFX implements Initializable {
    ...
    // --- Campos FXML ---
    // Campos de la Venta
    @FXML private TextField txtCliente; // Asumimos que es para un nombre o ID de cliente (opcional)
    @FXML private ComboBox<String> cmdMetodo;
    @FXML private TextField txtReceived; // Dinero recibido
    @FXML private TextField txtTotal;
    @FXML private Label lblChange; // Cambio / Faltante
    ...
    void onSave(ActionEvent event) {
        ...
        try {
            ...
            if (clientName.isEmpty()) {
                // 💡 Nota: Asegúrate de que tu ComboBox de método de pago (si existe) 
                // esté seteando el valor en newSale.setPaymentMethod() antes de este punto.
            ...
            // Navegar a la nueva ventana, obtener el controlador de la factura (QBFacturaFX)
            ...
            if (facturaController != null) {
                ...
                String successMessage = String.format("Venta guardada con éxito. ID: %s\nTotal: %s\nRecibido: %s\nCambio: %s. \nADVERTENCIA: No se pudo abrir la ventana de Factura.",
                    savedSale.getId(),
            ...
            // 6. Cerrar la ventana de venta actual
            Navigator.closeStage(event);

        } catch (NumberFormatException e) {
            showAlert(AlertType.ERROR, "Error de Datos", "Verifique que el total y el dinero recibido sean números válidos. Asegúrese de no usar caracteres no numéricos.");
            e.printStackTrace();
        } catch (Exception e) {
