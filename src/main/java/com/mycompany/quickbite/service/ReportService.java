    ...
    public SalesSummary generateSalesSummary(LocalDate start, LocalDate end) {
        ...
        Map<String, ProductStatsBuilder> statsMap = filteredSales.stream()
            ...
            .collect(Collectors.groupingBy(
                ...
                Collectors.reducing(
                    ...
                    item -> new ProductStatsBuilder(item.getQuantity(), item.getQuantity() * item.getUnitPrice()), // Mapeo
        ...
        List<QBReportesFX.ProductSalesStats> finalStats = statsMap.entrySet().stream()
            .map(entry -> {
                ...
                Product product = productService.getProductById(productId); // Obtener datos actuales del producto
                ...
                // IMPORTANTE: Dado que no podemos modificar variables en un stream de esta forma,
                // moveremos el cálculo de totalCost después del stream o usaremos un objeto atómico
        ...
        // Recalcular totalCost de manera simple (es lo más sencillo sin modificar el stream anterior)
        totalCost = finalStats.stream()
            .mapToDouble(s -> s.cost() * s.totalQuantitySold())
            .sum();
